...
export async function run(source : string, config: Config) : Promise<[Value, GlobalEnv, GlobalTypeEnv, string, WebAssembly.WebAssemblyInstantiatedSource]> {
  ...
  const wasmSource = `(module
    ...
    (func $encode_value (import "libmemory" "encode_value") (param i32) (param i32) (result i32))
    (func $decode_value (import "libmemory" "decode_value") (param i32) (result i32))
    ...
    (func $print_obj_in_mem (import "imports" "print_obj_in_mem") (result i32))
    (func $print_mem_used (import "imports" "print_mem_used") (result i32))
    ${globalImports}
    ${globalDecls}
    ${config.functions}
    ${compiled.functions}
