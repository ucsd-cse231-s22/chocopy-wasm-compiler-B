...
import { Program, Type, Value, SourceLocation, Modules } from './ast';
...
export async function run(modules : Modules, config: Config, astOpt: boolean = false, irOpt: boolean = false) : Promise<[Value, GlobalEnv, GlobalTypeEnv, string, WebAssembly.WebAssemblyInstantiatedSource, string]> {
  const parsed = parse(modules);
