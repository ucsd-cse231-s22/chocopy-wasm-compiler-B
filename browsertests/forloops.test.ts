import { assertPrint, assertRunTimeFail, assertTCFail } from './browser.test';
...
//3
   assertTCFail("for-loop no reset", `
class Range(object):
  current : int = 0
  min : int = 0
  max : int = 0
...
for i in cls:
  ...
  // 5
  assertPrint("for-loop no iterations", `
class Range(object):
  current : int = 0
  min : int = 0
  max : int = 0
...
for i in cls:
  ...
  //Comment below to shorten the test period
//   // 6
//   assertPrint(
...
// //20 FAIL make browser got stuck
