...
export function tc(env: GlobalTypeEnv, program: Program<null>): [Program<Type>, GlobalTypeEnv] {
  ...
  // add in the length value
  export function tcDef(env: GlobalTypeEnv, fun: FunDef<null>): FunDef<Type> {
    var locals = emptyLocalTypeEnv();
    locals.expectedRet = fun.ret;
    locals.topLevel = false;
    // const paramsSet: Set<string> = new Set<string>();
    fun.parameters.forEach(p => {
      if (locals.vars.has(p.name)) {
        throw new TypeCheckError(`Duplicate argument ${p.name} for function ${fun.name}`);
    ...
    fun.parameters.forEach(p => locals.vars.set(p.name, p.type)); // add p.value
    ...
    switch (expr.tag) {
      ...
      case "call":
        if (env.classes.has(expr.name)) {
          ...
          // CHANGE TO WORK WITH DEFAULT ARGS
          //
          // precheck length
          // if length is not equal
          //.  traverse required arguments until you get to the length + 1 argument
          //   if that argument is default
          //.     add all arguments from there to the end
          //.  check as normal
          //
          // example - arguments passed == 3, total arguments = 4 (2 normal 2 default)
          // go to fourth argument, if it is default just check types up until then
          // then check that the argument types and length still typecheck with the method
          ...
          for (index = 0; index < passedArgLength; index++) {
            if (tArgs[index].a !== argTypes[index]) {
              throw new TypeCheckError("Function call type mismatch: " + expr.name + " for argument " + index);
            }
        ...
        if (tObj.a.tag === "class") {
          if (env.classes.has(tObj.a.name)) {
            ...
            if (methods.has(expr.method)) {
              ...
              let index;
              for (index = 0; index < passedArgLength; index++) {
                if (!isAssignable(env, realArgs[index].a, methodArgs[index])) {
                  throw new TypeCheckError("Method call type mismatch: " + expr.method + " for argument " + index);
                }
              ...
              throw new TypeCheckError(`could not find method ${expr.method} in class ${tObj.a.name}`);
            }
          } else {
