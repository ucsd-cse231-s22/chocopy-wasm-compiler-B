...
import { Stmt, Expr, Type, UniOp, BinOp, Literal, Program, FunDef, VarInit, Class, SourceLocation, Parameter } from './ast';
...
export function tcDef(env: GlobalTypeEnv, fun: FunDef<SourceLocation>): FunDef<[Type, SourceLocation]> {
  ...
  return { ...fun, a: [NONE, fun.a], body: tBody, parameters, inits: tcinits };
}
        ...
        if (tObj.a[0].tag === "class") {
          if (env.classes.has(tObj.a[0].name)) {
            ...
            if (methods.has(expr.method)) {
              ...
              throw new TypeCheckError(`could not find method ${expr.method} in class ${tObj.a[0].name}`);
            }
          } else {
