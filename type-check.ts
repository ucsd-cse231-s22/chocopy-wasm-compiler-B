...
export function augmentTEnv(env: GlobalTypeEnv, program: Program<null>): GlobalTypeEnv {
  ...
  // let firstDefault = fun.parameters.find(p => p.value !== undefined);
  // if -1, there are no defaults
  // else, everything before this index is a non-default argument
  program.funs.forEach(fun => newFuns.set(fun.name, [fun.parameters.map(p => p.type), fun.ret])); // add defaultLength
