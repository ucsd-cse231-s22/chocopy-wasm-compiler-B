(module
  ...
  (func $encode_value (export "encode_value") (param $value i32) (param $is_pointer i32) (result i32)
    (local $true_value i32)
    ;; x shift to left and pad 0
    (local.get $value)
    (i32.const 1)
    (i32.shl)
    (local.set $true_value)
    ;; if it's a pointer pad 1 to it
    (local.get $is_pointer)
    (i32.const 1)
    (i32.eq)
    (if
      (then ;; if it's a pointer pad 1 to it
          (local.get $true_value)
          (i32.const 1)
          (i32.add)
          (return)
      )
  ...
  (func $value_is_pointer (export "value_is_pointer") (param $value i32) (result i32)
    (local.get $value)
    (i32.const 1)
    (i32.and) ;; 1 means pointer; 0 means normal value
  )
  ...
  (func (export "store") (param $addr i32) (param $offset i32) (param $val i32)
    ...
    ;; store the new value
    (i32.store (local.get $target_addr) (local.get $new_val))
  ...
  (func $get_refcount (export "get_refcount") (param $addr i32) (result i32)
    ...
    (call $decode_value)
    (local.set $addr)
    ;; (i32.add (local.get $addr) (i32.const 8))
